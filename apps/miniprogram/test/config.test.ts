...
describe('mini-program API configuration', () => {
  ...
  it('pins the privacy-capable base library and native TypeScript compiler', () => {
    ...
    expect(projectConfig.setting).not.toHaveProperty('packNpmManually')
    expect(projectConfig.setting).not.toHaveProperty('packNpmRelationList')
  })
  ...
  it('ships a bundled local contracts runtime without bare workspace imports', () => {
    const sourceRoot = resolve(import.meta.dirname, '../miniprogram')
    const runtimeBundle = readFileSync(resolve(sourceRoot, 'generated/contracts.js'), 'utf8')
    const sourceFiles = [
      'app.ts',
      'core/media-validation.ts',
      'core/resume.ts',
      'core/status-copy.ts',
      'pages/history/index.ts',
      'pages/upload/index.ts',
      'runtime/wx-upload.ts',
      'services/api-client.ts',
      'services/chunk-files.ts',
      'services/session-store.ts',
      'services/upload-runner.ts',
    ]

    expect(runtimeBundle).toContain('Generated by pnpm --filter @wx-upload/miniprogram')
    for (const sourceFile of sourceFiles) {
      expect(readFileSync(resolve(sourceRoot, sourceFile), 'utf8')).not.toContain(
