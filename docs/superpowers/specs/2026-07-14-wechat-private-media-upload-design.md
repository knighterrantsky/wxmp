...
系统采用一个代码仓库、两个首期应用和一个后续采集应用：

- `apps/miniprogram`：微信原生小程序，TypeScript。
- `apps/api`：Node.js + TypeScript + Fastify 业务后端。
- `apps/collector`：后续部署到采集服务器的任务消费者，首期只定义协议。
- PostgreSQL 保存身份、上传、分片、采集任务及审计数据。
...
- 采集服务器通过后端领取带租约的任务，并通过受租约保护的分段内容接口下载；R2 凭据始终只由业务后端持有。
- 完整对象默认长期保留；首版没有用户删除、公开预览、分享或自动清理完整对象的能力。

## 2. 背景与目标

用户打开小程序后直接进入素材上传页。系统自动完成微信身份登录，用户选择图片或视频、进行二次确认、查看真实上传进度，并在上传记录中看到素材从上传到采集完成的状态。
...
8. 为后续采集服务器提供任务领取、租约保护的分段内容读取、续租、成功确认和失败确认接口。
9. 保证 R2 全程私有，并提供可审计的身份、上传和采集状态。
...
- 不自动删除已采集的完整 R2 对象。
- 不在首期交付采集程序本身；首期交付其 API、数据结构和权限模型。
...
| 访问模型 | 用户无对象读取接口；采集服务器只能经 HTTPS 机器认证和有效租约调用后端分段内容接口 |
| 对象保留 | 完整对象不设自动删除生命周期 |
...
- PostgreSQL、R2 读写凭据、微信 AppSecret、JWT 私钥和采集凭据只存在于服务端秘密管理中。
- 采集服务器是受信机器主体，但不持有 R2 凭据。每次内容分段读取都必须同时通过机器 scope、任务归属和有效租约校验。
...
| Fastify API | 微信登录、令牌、鉴权、文件校验、R2 multipart、状态机、记录、采集任务、按租约流式读取、审计 | 向用户提供对象读取或公开链接 |
| PostgreSQL | 身份映射、会话、上传会话、分片、对象元数据、采集租约、幂等和审计 | 保存文件二进制 |
| Cloudflare R2 | 保存完整私有对象和未完成 multipart | 直接认证微信用户 |
| 采集服务器 | 领取任务、经后端分段下载、校验、最终采集、确认结果 | 持有 R2 凭据，修改或删除 R2 对象 |
...
sequenceDiagram
    ...
    A->>D: media=ready、session=completed、创建 collector job
    M->>A: GET /v1/uploads/{id}
    A-->>M: awaiting_collection
```
...
| `awaiting_collection` | 完整私有对象可用，等待采集 |
| `collecting` | 已被采集服务器领取 |
| `collected` | 采集服务器已确认成功 |
| `upload_failed` | 上传或 R2 完成失败 |
| `collection_failed` | 采集达到最大尝试次数、不可重试或被管理员取消 |
| `aborted` | 用户取消未完成上传 |
| `expired` | 上传会话超时 |

### 7.7 采集流程

1. API 在媒体对象变为 `ready` 的同一数据库事务内创建唯一 `collector_job`。
2. 采集服务器使用机器 API Key 领取 1–10 个任务。
3. PostgreSQL 先锁定机器凭据额度行，再使用 `FOR UPDATE SKIP LOCKED` 原子 claim，并签发 5 分钟租约；同一凭据并发请求合计不得超过配置的 1–10 个有效租约。
4. 任务返回内部 userId、昵称快照和文件元数据，不返回 R2 bucket、object key 或凭据。
5. 采集服务器携带机器凭据和租约 Token，通过内容接口按最大 8 MiB 的 HTTP Range 分段读取；API 校验租约后从 R2 流式转发。
6. 下载时校验每段长度和最终总字节数，并计算完整 SHA-256。
7. 长任务每 60 秒 heartbeat；单次 attempt 最长 30 分钟。
8. 成功后回写结果摘要；失败则按指数退避重试，最多 5 次，之后进入 `dead`。租约清理任务每分钟把过期 attempt 原子转为 `retry_wait` 或 `dead`，不会让最后一次崩溃永久停在 `leased`。

任务投递语义为 at-least-once。采集端必须以 `jobId` 作为下游幂等键，不能假设 exactly-once。
...
- Multipart ETag 不能当作完整文件 MD5；完整 SHA-256 由采集服务器下载后计算并回写。
...
- 对象存在且大小匹配：补齐数据库的 `completed/ready` 状态并创建唯一采集任务。
...
- 完成后用 `HEAD` 验证，再在单事务中更新媒体、上传会话与采集任务。
...
- Collector API Key 和用户 JWT 使用不同认证器与路由 scope。
- 所有 Collector 路由都要求机器 scope `collector:jobs`；内容读取、续租和结果路由还必须校验本次 attempt 的有效租约 Token。
...
- API 使用限定到单个 bucket 的读写凭据；小程序和采集服务器均不持有 R2 凭据。
- 微信 AppSecret、R2 Secret Access Key、JWT 私钥、数据库密码和 Collector API Key 均通过部署秘密注入。
- 日志不得记录 openid、session_key、完整 Token、R2 Secret、对象下载响应或用户文件内容。

### 10.4 网络与滥用防护

- 业务域名配置到微信小程序“服务器域名”白名单。
- TLS 最低 1.2，生产环境强制 HSTS。
- 登录、上传初始化、分片和采集接口分别限流。
...
| 采集进程崩溃 | 租约过期后任务重新可领取 |
| 数据库短暂不可用 | 不确认分片或采集成功，客户端/采集端按幂等规则重试 |
...
- `userId`、`mediaId`、`uploadId`、`partNumber`、`jobId`（存在时）
...
- 采集内容接口单次最多流式返回 8 MiB，不落本地磁盘；每个 Range 请求都重新校验租约，响应最迟在该请求开始时的租约截止时间终止。
- API 到 R2 的单分片超时必须短于客户端总超时，建议 150 秒。
- 初始单实例建议至少 2 vCPU、4 GiB 内存；数据库连接池上限 20。
- API 保持无状态，可在共享 PostgreSQL 前横向扩容。分片并发与任务 claim 不能只使用进程内锁。
...
7. Nginx 的分片路由设置 `client_max_body_size 16m`、`proxy_request_buffering off` 和不低于 210 秒的上游读取超时；采集内容路由设置 `proxy_buffering off`；其他 JSON 路由仍限制为 64 KiB。
...
- 对象键生成与路径逃逸防护。
- 状态机和错误码映射。
- JWT、Refresh Token 轮换与 Collector API Key 验证。
- 进度计算、退避和游标分页。

### 16.2 集成测试

- PostgreSQL 唯一约束、事务、幂等记录和 `SKIP LOCKED` claim。
...
6. 实现上传记录、游标分页和聚合状态。
7. 实现 Collector API、机器凭据、租约、重试与审计。
8. 完成 Nginx、生产部署、监控、备份和恢复演练。
9. 完成测试矩阵、微信真机验收和上线检查。
...
- 用户只能看到自己的上传记录。
- 完成上传会产生且只产生一个采集任务。
- 多采集进程不会在同一有效租约内领取同一任务；租约过期后可安全重领。
