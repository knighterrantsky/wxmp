...
> **历史基线文档，禁止作为当前上传链路的实施依据。** 当前架构已调整为“小程序只上传到业务服务器，服务器持久化落盘并通过数据库队列异步交付 R2”，客户端无自动重试且 UI 不展示 OSS/R2 状态。请以 [当前上传状态与一致性设计](../../design/upload-state-model.md)、[接口设计](../../api/media-upload-api.md) 和 [数据库设计](../../database/media-upload-database.md) 为准。
...
- 小程序进入后台时停止发起新请求，并按上传会话分别保存 `uploadId`、初始化/完成幂等键、源临时路径和文件元数据；不同文件不得共用或覆盖同一条断点记录。本地最多保留 5 条未完成会话，回到前台后依次查询服务端分片状态并恢复。
...
- 微信登录、令牌刷新、上传初始化和分片接口分别限流；刷新接口独立按 IP 限制为 30 次/分钟。
