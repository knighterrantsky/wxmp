...
自动部署脚本会注入 GHCR 中的 API、PostgreSQL、Nginx 镜像名与完整 commit SHA。手工预检时也必须显式提供这些非密钥值。`docker compose config` 会展开其他环境变量，输出可能包含密钥，因此只检查退出码，不保存或粘贴完整输出：

```bash
API_IMAGE=ghcr.io/<owner>/<repository>-api \
IMAGE_TAG=<40-character-commit-sha> \
POSTGRES_IMAGE=ghcr.io/<owner>/<repository>-api:postgres-<40-character-commit-sha> \
NGINX_IMAGE=ghcr.io/<owner>/<repository>-api:nginx-<40-character-commit-sha> \
...
正常发布由 GitHub Actions 完成：完整 CI 验证成功后，GitHub 托管 runner 构建 API 镜像，并将固定版本的 PostgreSQL、Nginx 镜像同步到 GHCR；只有显式开启生产变量后，服务器 self-hosted runner 才会按不可变 SHA 拉取和启动。服务器因此不需要直接访问 Docker Hub。首次配置与 runner 注册见 [GitHub Actions、GHCR 与生产自动部署](github-cicd.md)。
