...
从 `.env.example` 复制一份服务器本地 `.env`，权限设置为仅部署账号可读，并确保 `.env` 不进入 Git、备份日志或工单正文：
...
`docker compose config` 会展开环境变量，输出可能包含密钥，因此只检查退出码，不保存或粘贴完整输出：
...
启动依赖顺序为 PostgreSQL 健康、`migrate` 成功退出、API 健康、Nginx 启动。迁移失败时 API 不会启动；先查看迁移日志并修复，不能跳过：
...
docker compose --env-file /absolute/path/to/.env \
  -f /absolute/path/to/deploy/docker-compose.prod.yml \
  --profile maintenance \
...
4. API 镜像按不可变 tag 发布，回滚应用时不要回滚已写入数据的 migration 文件。
